...
import React, { useState, useEffect } from "react";
import { Pagination, InputLabel, FormControl, Select, MenuItem, List, Modal, Box, Typography, TextField, Button, Dialog, DialogContentText, DialogContent, DialogTitle, DialogActions, Grid } from "@mui/material";
import EmployeeCard from "../Presentation/EmployeeCard";
import useEmployeeDetails from "../../Hooks/useEmployeeDetails";
import { updateEmployee, disburseSalaries } from "../../Utils/httputils";
import { ToastContainer, toast, Bounce } from 'react-toastify';
...
const EmployeeList = () => {
  ...
  return (
      <div>
        ...
        {/* Department Filter */}
        <FormControl fullWidth style={{ marginBottom: '20px', color: "#8e44ad" }}>
          <InputLabel style={{ color: "#8e44ad" }}>Department</InputLabel>
          <Select
              value={selectedDepartment}
              onChange={handleDepartmentChange}
              label="Department"
        ...
        <FormControl fullWidth style={{ marginBottom: "20px" }}>
          <InputLabel style={{ color: "#8e44ad" }}>Title</InputLabel>
          <Select value={selectedTitle} onChange={handleTitleChange} label="Title">
            <MenuItem value="">
              <em>All</em>
            </MenuItem>
        ...
        <TextField
            ...
            style={{ marginBottom: '20px' }}
            InputProps={{
              style: { color: "#8e44ad" }, // Change text color
            }}
            InputLabelProps={{
              style: { color: "#8e44ad" }, // Change label color
            }}
        />
        <hr className="divider"></hr>

        {/* Employee Cards in a Grid Layout */}
        <Grid container spacing={2}>
          {paginatedEmployees.map((employee) => (
              <Grid item xs={12} sm={6} md={3} key={employee.employee_id}>
                <EmployeeCard
                    employee={employee}
                    isSelected={selectedEmployees.has(employee)}
                    onCheckboxChange={handleCheckboxChange}
        ...
        {selectedEmployees.size > 0 && (
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 2 }}>
              <Button
                  variant="contained"
                  style={{
                    background: "linear-gradient(to right, #8e44ad, #3498db)", // Blue gradient
                    color: "#fff",
                  }}
                  onClick={handleDisburse}
              >
